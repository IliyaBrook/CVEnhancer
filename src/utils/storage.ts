...
const SELECTED_JSON_FILE_KEY = 'cvenhancer_selected_json_file';

export const saveSelectedJsonFile = (filename: string): void => {
	try {
		localStorage.setItem(SELECTED_JSON_FILE_KEY, filename);
	} catch (error) {
		console.error('Failed to save selected JSON file:', error);
	}
};

export const loadSelectedJsonFile = (): string => {
	try {
		const stored = localStorage.getItem(SELECTED_JSON_FILE_KEY);
		return stored || '';
	} catch (error) {
...
export const clearSelectedJsonFile = (): void => {
	try {
		localStorage.removeItem(SELECTED_JSON_FILE_KEY);
	} catch (error) {
		console.error('Failed to clear selected JSON file:', error);
	}
};
