...
const createResumeEnhancementPrompt = (jobTitle?: string): string => {
  ...
  if (config?.experience) {
    ...
    if (config.experience.maxBulletLength !== null && config.experience.maxBulletLength !== undefined) {
      experienceConstraints.push(
        `CRITICAL: Each bullet point MUST be maximum ${config.experience.maxBulletLength} characters. DO NOT EXCEED THIS LIMIT.`
      );
    } else {
      experienceConstraints.push(
        `IMPORTANT: Write full, detailed bullet points. DO NOT truncate or abbreviate unless necessary.`
      );
    }
    if (config.experience.requireActionVerbs) {
      experienceConstraints.push(
        `REQUIRED: Start each bullet point with a strong action verb (e.g., "Developed", "Implemented", "Managed", "Led")`
      );
    }
    if (config.experience.metricsLevel) {
      const metricsInstructions = {
        ...
        moderate:
          'Actively look for and include quantifiable metrics and achievements (percentages, numbers, sizes, etc.)',
...
const createOllamaStep2Prompt = (jobTitle?: string): string => {
  ...
  if (config?.experience) {
    ...
    if (config.experience.bulletPointsPerJob !== null && config.experience.bulletPointsPerJob !== undefined) {
      constraints.push(
        `CRITICAL: Each job MUST have EXACTLY ${config.experience.bulletPointsPerJob} bullet points (duties). This is a STRICT requirement.`
      );
    }
    ...
    if (config.experience.maxBulletLength !== null && config.experience.maxBulletLength !== undefined) {
      ...
      constraints.push(
        `IMPORTANT: Write full, complete bullet points. DO NOT truncate or abbreviate. Better to be detailed than brief.`
      );
    }
...
const enhanceWithOpenAI = async (
    ...
    messages: [
      {
        role: 'system',
        content:
          'You are a professional resume enhancement AI. Extract ONLY information from the provided resume. Do not invent or fabricate any data. Return only valid JSON matching the provided schema.',
      },
      {
        role: 'user',
        content: userContent,
  ...
  if ('error' in result) {
    throw new Error(result.error?.toString() || 'OpenAI API error');
  }

  return enforceResumeConstraints(result.data);
};
...
const enhanceWithClaude = async (
  ...
  const result = await store.dispatch(
    aiApi.endpoints.enhanceWithClaude.initiate({
      body: claudeBody,
  ...
  if ('error' in result) {
    throw new Error(result.error?.toString() || 'Claude API error');
  }
  ...
  const ollamaRequest = async (prompt: string, stepName: string, contextText?: string, retries = 2): Promise<any> => {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        const result = await store.dispatch(
          aiApi.endpoints.generateWithOllama.initiate({
            body: ollamaBody,
            endpoint,
          })
        );
