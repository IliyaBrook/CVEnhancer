...
import { setSelectedJsonFile, setJsonFiles, setJsonFilesLoading, setJsonFilesError } from '@/store/slices';
...
export function JsonFileSelector({ onFileSelect }: JsonFileSelectorProps) {
  ...
  const { files, loading, error } = useAppSelector(state => state.app.jsonFileSelector);
  const selectedFile = useAppSelector(state => state.app.selectedJsonFile);
  ...
  const loadFileList = async () => {
    try {
      const response = await axios.get<Array<{ name: string; displayName: string }>>('/api/json-files');
      dispatch(setJsonFiles(response.data));

      if (selectedFile && response.data.some(f => f.name === selectedFile)) {
        void loadFile(selectedFile);
      }
    } catch (err) {
      console.error('Error loading file list:', err);
      dispatch(setJsonFilesError('Failed to load file list'));
    }
  };

  const loadFile = async (filename: string) => {
    dispatch(setJsonFilesLoading(true));
    dispatch(setJsonFilesError(''));
    ...
    try {
      ...
      dispatch(setJsonFilesError('Failed to load file'));
    } finally {
      dispatch(setJsonFilesLoading(false));
    }
  };
  ...
  return (
    <div className="h-fit rounded-2xl border-2 border-violet-200 bg-white p-6 shadow-xl transition-all duration-300 hover:shadow-2xl">
