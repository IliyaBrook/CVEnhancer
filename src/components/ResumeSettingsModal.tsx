...
export const ResumeSettingsModal: React.FC<ResumeSettingsModalProps> = ({isOpen, onClose}) => {
	...
	const [localConfig, setLocalConfig] = useState<ResumeConfig>(config);
		...
		if (excludeJobInput.trim()) {
			setLocalConfig({
				...
				experience: {
					...
					exclude: [...localConfig.experience.exclude, excludeJobInput.trim()]
				}
			});
			setExcludeJobInput('');
		}
	...
	const handleRemoveExcludeJob = (index: number) => {
		setLocalConfig({
			...
			experience: {
				...localConfig.experience,
				exclude: localConfig.experience.exclude.filter((_, i) => i !== index)
			}
		});
	};
	...
	const handleAddExcludeEducation = () => {
		if (excludeEducationInput.trim()) {
			setLocalConfig({
				...localConfig,
				education: {
					...localConfig.education,
					exclude: [...localConfig.education.exclude, excludeEducationInput.trim()]
				}
			});
	...
	const handleRemoveExcludeEducation = (index: number) => {
		setLocalConfig({
			...
			education: {
				...localConfig.education,
				exclude: localConfig.education.exclude.filter((_, i) => i !== index)
			}
		});
	};
	...
	return (
		<div className='fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
			<div
				className='bg-gradient-to-br from-white to-violet-50/30 rounded-2xl shadow-2xl border border-violet-100/50 max-w-3xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col'>
				<div
					className='sticky top-0 bg-gradient-to-r from-violet-50 to-purple-50 border-b-2 border-violet-200 px-6 py-5 flex items-center justify-between backdrop-blur-sm'>
					<div className='flex items-center gap-3'>
						...
						<h2
							className='text-2xl font-bold bg-gradient-to-r from-violet-600 to-purple-600 bg-clip-text text-transparent'>Resume
							Settings</h2>
					</div>
					<button
						onClick={onClose}
					...
					>
						<svg className='w-6 h-6 transition-transform group-hover:rotate-90 duration-300' fill='none'
				...
				<div className='overflow-y-auto px-6 py-6 space-y-8'>
					<div className='border-b-2 border-violet-100 pb-6'>
						<div className='flex items-center gap-2 mb-6'>
							...
							<h3
								className='text-xl font-bold bg-gradient-to-r from-violet-700 to-purple-600 bg-clip-text text-transparent'>Experience
								Settings</h3>
						</div>
						...
						<div className='grid grid-cols-2 gap-4 mb-6'>
							<div>
								...
								<input
									type='number'
									min='1'
									value={localConfig.experience.maxJobs}
									onChange={(e) => setLocalConfig({
										...localConfig,
										experience: {...localConfig.experience, maxJobs: parseInt(e.target.value) || 1}
									})}
							...
							<div>
								...
								<input
									type='number'
									min='1'
									value={localConfig.experience.bulletPointsPerJob}
									onChange={(e) => setLocalConfig({
										...localConfig,
										experience: {...localConfig.experience, bulletPointsPerJob: parseInt(e.target.value) || 1}
									})}
									className='w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-violet-500/20 focus:border-violet-500 transition-all duration-200 bg-white hover:border-violet-300 font-medium text-gray-900'
								/>
							</div>
							...
							<div>
								...
								<input
									...
									value={localConfig.experience.maxBulletLength || ''}
									onChange={(e) => setLocalConfig({
										...localConfig,
										experience: {
											...localConfig.experience,
											maxBulletLength: e.target.value ? parseInt(e.target.value) : null
										}
									})}
							...
							<div>
								...
								<select
									value={localConfig.experience.metricsLevel}
									onChange={(e) => setLocalConfig({
										...localConfig,
										experience: {...localConfig.experience, metricsLevel: e.target.value as 'low' | 'moderate' | 'high'}
									})}
						...
						<div className='mb-4'>
							<label
								className='flex items-center gap-3 p-3 rounded-xl hover:bg-violet-50 transition-colors cursor-pointer group'>
								<input
									type='checkbox'
									checked={localConfig.experience.requireActionVerbs}
									onChange={(e) => setLocalConfig({
										...localConfig,
										experience: {...localConfig.experience, requireActionVerbs: e.target.checked}
									})}
						...
						<div className='mb-6'>
							<label
								...
								<input
									type='checkbox'
									checked={localConfig.experience.avoidDuplicatePoints}
									onChange={(e) => setLocalConfig({
										...localConfig,
										experience: {...localConfig.experience, avoidDuplicatePoints: e.target.checked}
									})}
									className='w-5 h-5 rounded-lg text-violet-600 focus:ring-4 focus:ring-violet-500/20 border-2 border-gray-300 cursor-pointer transition-all'
								/>
						...
						<div>
							...
							{localConfig.experience.exclude.length > 0 && (
								<div className='flex flex-wrap gap-2'>
									{localConfig.experience.exclude.map((job, index) => (
										<span
											key={index}
					...
					<div className='border-b-2 border-violet-100 pb-6'>
						<div className='flex items-center gap-2 mb-6'>
							...
							<h3
								className='text-xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent'>Skills
								Settings</h3>
						</div>
						...
						<div className='grid grid-cols-2 gap-4'>
							<div>
								...
								<input
									type='number'
									min='1'
									value={localConfig.skills.categoriesLimit}
									onChange={(e) => setLocalConfig({
										...localConfig,
										skills: {...localConfig.skills, categoriesLimit: parseInt(e.target.value) || 1}
									})}
							...
							<div>
								...
								<input
									type='number'
									min='1'
									value={localConfig.skills.skillsPerCategory}
									onChange={(e) => setLocalConfig({
										...localConfig,
										skills: {...localConfig.skills, skillsPerCategory: parseInt(e.target.value) || 1}
									})}
									className='w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all duration-200 bg-white hover:border-emerald-300 font-medium text-gray-900'
								/>
							</div>
					...
					<div>
						<div className='flex items-center gap-2 mb-6'>
							...
							<h3
								className='text-xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent'>Education
								Settings</h3>
						</div>
						...
						<div className='grid grid-cols-2 gap-4 mb-6'>
							<div>
								...
								<input
									type='number'
									min='1'
									value={localConfig.education.maxEntries}
									onChange={(e) => setLocalConfig({
										...localConfig,
										education: {...localConfig.education, maxEntries: parseInt(e.target.value) || 1}
									})}
							...
							<div>
								...
								<select
									value={localConfig.education.placement}
									onChange={(e) => setLocalConfig({
										...localConfig,
										education: {...localConfig.education, placement: e.target.value as 'main-content' | 'sidebar'}
									})}
						...
						<div className='mb-6'>
							<label
								...
								<input
									type='checkbox'
									checked={localConfig.education.showDates}
									onChange={(e) => setLocalConfig({
										...localConfig,
										education: {...localConfig.education, showDates: e.target.checked}
									})}
						...
						<div>
							...
							{localConfig.education.exclude.length > 0 && (
								<div className='flex flex-wrap gap-2'>
									{localConfig.education.exclude.map((edu, index) => (
										<span
											key={index}
					...
					<div>
						<div className='flex items-center gap-2 mb-6'>
							...
							<h3
								className='text-xl font-bold bg-gradient-to-r from-rose-600 to-red-600 bg-clip-text text-transparent'>PDF
								Export Settings</h3>
						</div>
						
						<div>
							<label
								className='flex items-center gap-3 p-3 rounded-xl hover:bg-rose-50 transition-colors cursor-pointer group'>
								<input
									type='checkbox'
									checked={localConfig.pdf.singlePageExport}
									onChange={(e) => setLocalConfig({
										...localConfig,
										pdf: {...localConfig.pdf, singlePageExport: e.target.checked}
									})}
								...
								<div className='flex flex-col'>
									...
									<span
										className='text-xs text-gray-500 mt-0.5'>Export PDF as one continuous page without page breaks</span>
								</div>
							</label>
						</div>
					</div>
				...
				<div
					className='sticky bottom-0 bg-gradient-to-r from-violet-50 to-purple-50 border-t-2 border-violet-200 px-6 py-5 flex justify-end gap-4 backdrop-blur-sm'>
					<button
						onClick={onClose}
